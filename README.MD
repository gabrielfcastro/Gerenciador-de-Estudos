...
> Sou formado em Engenharia da Computação pela UnB e comecei a usar esse projeto em 2025 para me ajudar a estudar para concurso e organizar meu tempo. À medida que sentir necessidade, vou adicionando novas funcionalidades — essa é a terceira versão.
...
- **Navegação por período** — avançar/voltar entre semanas, dias e meses anteriores mantendo o mesmo filtro
...
- **Cronograma semanal** — planejamento fixo por dia da semana, com matérias arrastáveis para cada dia
...
HTML · CSS · JavaScript (módulos ES)"]
    ...
    classDef frontend fill:#F3F0FF,stroke:#7C3AED,stroke-width:2px,color:#111827
    classDef backend fill:#EFF6FF,stroke:#2563EB,stroke-width:2px,color:#111827
    classDef database fill:#ECFDF5,stroke:#059669,stroke-width:2px,color:#111827
    classDef actor fill:#FFFFFF,stroke:#6B7280,stroke-width:2px,color:#111827

    class user actor
    class front frontend
    class api,service,repo backend
...
|---|---|---|
| Frontend | `index.html` + `style.css` + `js/*.js` | Interface, estado local, chamadas HTTP |
...
├── .gitignore
├── README.md
├── requirements.txt       # dependências de desenvolvimento (pytest) — runtime não usa nada externo
├── src/
│   ├── app.py             # Servidor HTTP + Roteador declarativo
...
│   ├── style.css          # Estilos
│   └── js/
│       ├── main.js        # Bootstrap: registra listeners globais e inicializa a UI
│       ├── api.js         # Client HTTP — único ponto de acesso à API
│       ├── csel.js        # Componente reutilizável de dropdown de matéria
│       ├── categories.js  # Estado das matérias (fonte única de verdade + pub/sub)
│       ├── timer.js       # Cronômetro, bloco de estudo e alarme
│       ├── sessions.js    # Gráficos, estatísticas e histórico de sessões
│       ├── tasks.js       # Kanban de tarefas
│       ├── cronograma.js  # Planejamento semanal
│       └── utils.js       # Formatação de datas, durações e labels
└── tests/
...
| PUT | `/api/sessions/:id` | Edita horário/categoria/nota de uma sessão |
...
| PUT | `/api/schedule/:id` | Move entrada para outro dia |
...
| Frontend | HTML + CSS + JavaScript puro (módulos ES nativos, sem bundler) |
...
Em runtime, nenhuma dependência externa de Python é usada — tudo que o backend precisa já vem na biblioteca padrão. O `requirements.txt` existe apenas para instalar o `pytest` em ambiente de desenvolvimento.
